...
public class JoinRoomReader {
	...
	public static void read(ChannelHandlerContext ctx, byte[] payload) {
		...
		//ByteBuf onlyMobPacket = OnlyMobConfig(joiningPlayer, room);
		//joiningPlayer.getPlayerCtxChannel().eventLoop().execute(() -> {
			//joiningPlayer.getPlayerCtx().writeAndFlush(onlyMobPacket);
		//});
		
		//joiningPlayer.getPlayerCtxChannel().eventLoop().schedule(() -> {
			//room.submitAction(() -> OnlyMobConfig(joiningPlayer, room));
		//},1000,TimeUnit.MILLISECONDS);

	}

	private static void OnlyMobConfig(PlayerSession joiningPlayer, GameRoom room) {
		Optional<MobileRoom> isAnyMob = room.getRoomMobiles().getAll().values().stream().filter(m -> !m.isEnabled())
				.findFirst();

		ByteBuf onlyMobPayload = joiningPlayer.getPlayerCtx().alloc().buffer();

		if (isAnyMob.isPresent()) {
			onlyMobPayload = MobilePacketWriter.writeMobUpdate(joiningPlayer.getCurrentRoom());

		} else {
			onlyMobPayload.writeByte(0);
		}

		System.out.println("--ENVIANDO ONLYMOB--");
		ByteBuf onlyMobPacket = PacketUtils.generatePacket(joiningPlayer, 0x9990, onlyMobPayload, false);
		joiningPlayer.getPlayerCtxChannel().writeAndFlush(onlyMobPacket);
	}
	...
	public static ByteBuf writeJoinRoomSuccess(GameRoom room, PlayerSession ps) {
		...
		for (Map.Entry<Integer, PlayerSession> entry : room.getPlayersBySlot().entrySet()) {
			...
			buffer.writeBytes(Utils.resizeBytes(player.getGuild().getBytes(StandardCharsets.UTF_8), 8));
			...
			// buffer.writeBytes(new byte[] { 01 });//PU
			buffer.writeByte(player.getIsPowerUser());// tem pu
		...
		String msg = ServerConfig.getInstance().getGameServerRoomMent();
		String motd = msg != null ? msg : "*Jogue Limpo!";
