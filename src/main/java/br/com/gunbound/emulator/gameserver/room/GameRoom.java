...
import br.com.gunbound.emulator.gameserver.packets.readers.room.gameplay.GamePlayerDeadReader;
import br.com.gunbound.emulator.gameserver.packets.writers.MobilePacketWriter;
import br.com.gunbound.emulator.gameserver.packets.writers.RoomWriter;
import br.com.gunbound.emulator.gameserver.playdata.MapData;
import br.com.gunbound.emulator.gameserver.playdata.MapDataLoader;
import br.com.gunbound.emulator.gameserver.playdata.SpawnPoint;
import br.com.gunbound.emulator.gameserver.room.model.enums.GameMode;
import br.com.gunbound.emulator.gameserver.room.onlymob.OnlyMobService;
import br.com.gunbound.emulator.gameserver.room.onlymob.RoomMobiles;
...
public class GameRoom {
	...
	// Usado para o comando onlymob
	
	private final RoomMobiles roomMobiles = new RoomMobiles();
	...
	// team Vencedora = usado na playlog
	int winnerTeam = -1;

	// seta o horario de inicio da partida
	...
	private EventLoop roomEventLoop; // eventloop atribuido a sala
	
	//Flags auxiliares para modos de jogos
	private final AtomicBoolean isRandomTeamsEnabled = new AtomicBoolean(false);
	...
	public int getInGameMapId() {
		return inGameMapId;
	}

	public void setInGameMapId(int inGameMapId) {
		this.inGameMapId = inGameMapId;
	}
    ...
    public RoomMobiles getRoomMobiles() {
        return roomMobiles;
    }
	...
	public boolean isAtagRoom() {
		return GameMode.fromId(getGameMode()).equals(GameMode.TAG);
	}
	...
	public EventLoop getRoomEventLoop() {
		return roomEventLoop;
	}
	
	
	//***********************SECAO DAS FLAGS*************************
    /**
     * Alterna o estado da flag de times aleatórios.
     * @return true se agora está ATIVADO, false se está DESATIVADO.
     */
    public boolean toggleRandomTeamsEnabled() {
        boolean currentState = isRandomTeamsEnabled.get();
        isRandomTeamsEnabled.set(!currentState);
        return !currentState; // Retorna o novo estado
    }
    
    public boolean isRandomTeamsEnabled() {
        return isRandomTeamsEnabled.get();
    }
    
    /**
     * Lógica principal para embaralhar os times.
     * Pega todos os jogadores, embaralha a lista e os reatribui aos times.
     * Este método DEVE ser chamado de dentro do gameLogicExecutor.
     */
    public void shuffleTeams() {
        ...
        // 1. Coleta todos os jogadores
        List<PlayerSession> playerList = new ArrayList<>(playersBySlot.values());
        
        // 2. Embaralha a lista
        Collections.shuffle(playerList);
        
        // 3. Reatribui os times
        // A lógica divide a lista ao meio. Math.ceil garante que
        // se houver um número ímpar (ex: 5 players), o Time A (0) fique com 3 e o Time B (1) com 2.
        int teamASize = (int) Math.ceil(playerList.size() / 2.0);
        
        for (int i = 0; i < playerList.size(); i++) {
            PlayerSession player = playerList.get(i);
            int newTeam = (i < teamASize) ? 0 : 1;
  ...
  //***********************SECAO DAS FLAGS*************************
	...
	/**
	 * Tenta ativar o flag de fim de jogo. Garante que o endgame só será processado
	 * uma única vez por partida. --------------- O método compareAndSet(false,
	 * true) verifica se o valor atual é false: Se for, coloca como true e retorna
	 * true (indica que você foi o primeiro a disparar). Se já estiver true, retorna
	 * false (outro fluxo já disparou/finalizou antes). ----------------
	 * 
	 ...
	 * processe o fim normalmente. Deve ser chamado após preparar a sala para uma
	 * nova partida.
	 */
    ...
    /**
     * Reseta os estados da sala para um novo jogo.
     */
    public void resetRoomForNewGame() {
		// Caso a sala teve um jogo anterior reseta a flag de endGame
		this.resetEndGameFlag();
        this.startTime = null;
        this.resultGameBySlot.clear();
        this.isGameStarted = false;
		// reseta o time vencedor ao iniciar um jogo
		this.setWinnerTeam(-1);
	...
	/**
	 * Realiza toda a checagem se o jogo atingiu condição de fim. Se sim, retorna o
	 * time vencedor.
	 * 
	 * Regras: - Em modo SCORE: O time que zerar o placar perde. Retorna o ID do
	 * time vencedor. - Em outros modos: O time que não tem mais jogadores vivos
	 * perde. Retorna ID do vencedor.
	 * 
	...
	public int isFinalGame() {

		int winnerTeam = this.checkGameEndAndGetWinner();
		if (winnerTeam != -1 && this.tryTriggerEndGame()) {
			return winnerTeam;
		}
	...
	public void processPlayerExitValidations() {

		// se o jogo nao ta sendo "jogado" nao tem porque verificar isso
		if (!isGameStarted()) {
			return;
		}

		int winnerTeam = this.checkGameEndAndGetWinner();
		// Se encontramos um vencedor E a sequência de fim de jogo ainda não foi
		// iniciada...
		if (winnerTeam != -1 && this.tryTriggerEndGame()) {
			// ...então, anunciamos o placar final.
			System.out.println("Partida finalizada na sala " + this.getRoomId() + ". Time vencedor: " + winnerTeam);
			this.submitAction(() -> GamePlayerDeadReader.announceFinalScore(this, winnerTeam));
		}
	}
	...
	public int addPlayer(PlayerSession player) {
		...
		if (this.roomEventLoop == null) {
			// Na primeira vez que um jogador entra, capturamos o EventLoop.
			// Como um EventLoop gerencia múltiplos canais, ele continuará válido
			// mesmo que este jogador específico saia.
			this.roomEventLoop = player.getPlayerCtx().channel().eventLoop();
		}
	...
	/**
	 * Passa para um novo jogador a key da sala.
	 */
	public boolean passKeyRoomMaster(PlayerSession newMaster) {
		if (newMaster == null) {
			return false;
		} else {
			// v
			Integer roomId = newMaster.getCurrentRoom().getRoomId();

			if (roomId.equals(this.roomId)) {
				this.roomMaster = playersBySlot.get(getSlotPlayer(newMaster));
				passKeyHostMigration();
	...
	public void startGame(byte[] payload) {
		...
		// seta o horario que iniciou a partida
		...
		if (this.mapId == 0) {
			...
			this.inGameMapId = Utils.randomMap();
		} else {
			this.inGameMapId = this.mapId;
		}

		MapData mapData = MapDataLoader.getMapById(this.inGameMapId);
		if (mapData == null) {
			System.err.println("Mapa com ID " + this.inGameMapId + " não encontrado!");
			return;
		}
		
		// Define a Team caso as flags estejam para randomizar
		//System.out.println("[DEBUG] isRandomTeamsEnabled >> " + isRandomTeamsEnabled());
		//if(isRandomTeamsEnabled()) {
		//	shuffleTeams();
		//}
		...
		int isASide = (this.gameSettings >> 16) & 1;// Verifica o bit menos significativo
		// boolean isASide = true;
		System.out.println("Map Side (A=0, B=1): " + isASide);
		...
		//Flag para verificar se a sala ta com onlymob configurado
		
		boolean isOnlyMobRoom = this.getRoomMobiles().isOnlyMobEnabled();
		...
		for (Map.Entry<Integer, PlayerSession> entry : playersBySlot.entrySet()) {
			...
			// seta para "vivo" e reseta o dead time ao iniciar o jogo
			// player.setIsAlive(1);
			player.resetForNewGame();

			readyStatusBySlot.put(slot, false); // Todo jogador volta como "não pronto".
			...
			if (player.getRoomTankPrimary() == 0xFF) {
				// Jogador escolheu RANDOM, então sorteamos um.
				player.setInGameTankPrimary(Utils.randomMobile());
			} else {
				// Jogador escolheu um mobile específico.
				player.setInGameTankPrimary(player.getRoomTankPrimary());
			}

			if (isAtagRoom()) {//so randomiza o segundo se for tag
				// Define o Mobile Secundário "Em Jogo"
				if (player.getRoomTankSecondary() == 0xFF) {
					// Jogador escolheu RANDOM, então sorteamos um.
					player.setInGameTankSecondary(Utils.randomMobile());
				} else {
					// Jogador escolheu um mobile específico.
				...
				OnlyMobService.validateAndApply(player, this);
			}

			// Caso nao esteja para randomizar so copia o team do player
			if(!isRandomTeamsEnabled()) {
				player.setInGameTeam(player.getRoomTeam());
			}
		...
		Map<Integer, Integer> turnOrderBySlot = new HashMap<>();
		List<Integer> slots = new ArrayList<>(playersBySlot.keySet());
		Collections.shuffle(slots);

		for (int i = 0; i < slots.size(); i++) {
		    turnOrderBySlot.put(slots.get(i), i);
		}
		...
		if (isAscoreRoom()) {
			...
			// area comentada dia 11/08/2025 -> bug no contador das partidas
			// verifica se as equipes estao desbalanceadas (Ex: Gm start 3x2)
			// size = (size % 2 == 0) ? size : size + 1;

			setScoreTeamA(teamACount + 1);
			setScoreTeamB(teamBCount + 1);
			...
			System.out.println("ScoreTeam B: " + getScoreTeamB());
		}

		// 6. Constrói e envia o pacote de início
		ByteBuf startPayload = Unpooled
				.wrappedBuffer(RoomWriter.writeGameStartPacket(this, turnOrderBySlot, playerSpawns, payload));
			...
			try {
				...
				submitAction(() -> playerInRoom.getPlayerCtxChannel().writeAndFlush(notifyPacket)
						.addListener((ChannelFutureListener) future -> {
							if (!future.isSuccess()) {
								System.err
										.println("FALHA AO ENVIAR PACOTE DE TÚNEL para: " + playerInRoom.getNickName());
	...
	public void broadcastMobileConfig() {
		ByteBuf notifyPayload = MobilePacketWriter.writeMobUpdate(this);
		...
		for (PlayerSession player : playersBySlot.values()) {
			try {
				...
				// Envia o pacote criptografado
				// int txSum = player.getPlayerCtx().attr(GameAttributes.PACKET_TX_SUM).get();
				ByteBuf finalPacket = PacketUtils.generatePacket(player, 0x9990,
						Unpooled.wrappedBuffer(notifyPayload), false);
	...
	public void notifyPlayerLeft(int leftPlayerSlot, boolean wasHost, Runnable onComplete) {
		// O payload é um short (2 bytes) contendo o ID do slot.
		ByteBuf payload = Unpooled.buffer().writeShortLE(leftPlayerSlot);

		int totalPlayers = getPlayersBySlot().size();
		if (totalPlayers == 0) {
			if (onComplete != null)
				onComplete.run();
			return;
							...
							.addListener((ChannelFutureListener) future -> {
								if (future.isSuccess()) {
									...
									if (wasHost) {
										...
										// notifyHostMigration();
										//this.notifyHostMigration();
										this.sendHostMigration();
									}
								}
							});
	...
	private void passKeyHostMigration() {

		sendHostMigration();
	}

	public void sendHostMigration() {
		...
		Collection<PlayerSession> recipients = new ArrayList<>(getPlayersBySlot().values());
		for (PlayerSession playerInRoom : recipients) {
			System.out.println("[DEBUG] Notificando novomaster para >>" + playerInRoom.getNickName());
			// int playerTxSum =
			// playerInRoom.getPlayerCtx().attr(GameAttributes.PACKET_TX_SUM).get();
			...
			// Envia o pacote individualmente. Comentado em 26/11/2025 race condition
			//playerInRoom.getPlayerCtxChannel().eventLoop().execute(() -> {
				//playerInRoom.getPlayerCtxChannel().writeAndFlush(notifyPacket);
			//});

			submitAction(() -> playerInRoom.getPlayerCtxChannel().eventLoop().execute(() -> {
				MessageBcmReader.printMsgToPlayer(playerInRoom, "New Master: " + getRoomMaster().getNickName());
			}), playerInRoom.getPlayerCtx());
	...
	// Enfileira ação sem contexto
	public void submitAction(Runnable action) {
		actionQueue.add(new QueuedAction(action, null));
		processNext(); // Tenta processar (só dispara se não estiver em processamento)
	}
	...
	private void nextAction() {
		...
		if (act != null) {
			...
			if (act.ctx == null) {
				System.out.println("ENTROU NO IF DE ACTION SEM O CTXXX");
				try {
					act.task.run();
				} catch (Exception e) {
