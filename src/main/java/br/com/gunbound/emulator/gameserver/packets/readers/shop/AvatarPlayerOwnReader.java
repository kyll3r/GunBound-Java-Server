...
public class AvatarPlayerOwnReader {
	...
	public static void read(ChannelHandlerContext ctx, byte[] payload) {
		...
		List<ChestDTO> playerItems = chestService.getPlayerItems(player);

		// Usa o Collectors para mapear de ChestDTO para PlayerAvatar
		List<PlayerAvatar> playerAvatars = playerItems.stream().map(chestDTO -> new PlayerAvatar(chestDTO))
				.collect(Collectors.toList());

		// Adiciona os PlayerAvatars à lista do jogador
		 player.getPlayerAvatars().addAll(playerAvatars);
		
		/*
		 * 
		 * Integer puItem = serverConfig.getGameServerSuperUserItem(); puItem = puItem
		 * != null ? puItem : 204801;
		 * 
		 * // valida se é um PowerUser int isPowerUser =
		 * Utils.hasActivePowerUserItem(playerItems);
		 * player.setIsPowerUser(isPowerUser);
		 * 
		 * playerItems.forEach(avatar -> { // Verifica se a data de expiração do avatar
		 * é nula ou se é uma data futura boolean isValid = avatar.getExpire() == null
		 * ||
		 * Utils.convertToLocalDateTime(avatar.getExpire()).isAfter(LocalDateTime.now())
		 * ;
		 * 
		 * if (isValid) { // Adiciona o avatar se ele estiver dentro da validade (ou for
		 * eterno) player.getPlayerAvatars().add(new PlayerAvatar(avatar)); } });
		 */
