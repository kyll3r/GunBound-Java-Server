...
public final class RoomWriter {
	...
	public static ByteBuf writeNotifyPlayerJoinedRoom(PlayerSession newPlayer) {
		...
		buffer.writeByte(newPlayer.getIsPowerUser());// tem pu
	...
	public static ByteBuf writeRoomList(Collection<GameRoom> rooms) {
		...
		for (GameRoom room : rooms) {
			...
			buffer.writeByte(room.getRoomMaster().getIsPowerUser());// tem pu
	...
	public static byte[] writeGameStartPacket(GameRoom room, Map<Integer, Integer> turnOrderBySlot,
		...
		buffer.writeByte(room.getInGameMapId());// modificado em 16/11/25
		...
		for (Map.Entry<Integer, PlayerSession> entry : room.getPlayersBySlot().entrySet()) {
			...
			// buffer.writeByte(player.getInGameTeam());

			buffer.writeByte(player.getInGameTankPrimary());
			buffer.writeByte(player.getInGameTankSecondary());

			// buffer.writeByte((player.getRoomTankPrimary()) == 0xFF ? Utils.randomMobile()
			// : player.getRoomTankPrimary());
			// buffer.writeByte(player.getRoomTankSecondary() == 0xFF ? Utils.randomMobile()
			// : player.getRoomTankSecondary());
			...
			buffer.writeShortLE(turnOrderBySlot.get(slot));
		}

		 //buffer.writeBytes(new byte[] { (byte) 0xFF, (byte) 0xFF });// Evento ativa
		// com FF00
		
		Integer eventTrigger = serverConfig.getGameServerEventTrigger();
		eventTrigger = eventTrigger != null ? eventTrigger : 0;
		
		System.out.println("eventTrigger >>> " + eventTrigger);

			if (eventTrigger > 0 && Utils.activeEvent()) {
				buffer.writeBytes(new byte[] { (byte) 0x00, (byte) 0x00 });// Evento ativa com FF00
			} else {
				buffer.writeBytes(new byte[] { (byte) 0xFF, (byte) 0xFF });
			}
